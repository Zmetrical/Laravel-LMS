$(document).ready(function () {
    ...
    // Action badge map (primary/secondary only)
    const ACTION_BADGES = {
        'viewed':    'badge-secondary',
        'submitted': 'badge-primary',
        'attempted': 'badge-secondary',
    ...
    // ── DataTable ────────────────────────────────────────────────────────────
    const dataTable = $('#studentAuditTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: API_ROUTES.getStudentLogs,
            type: 'GET',
            data: function (d) {
                d.student_number = $('#studentFilter').val();
        ...
        language: {
            emptyTable:   'No student activity logs found',
    ...
    // ── Filters ──────────────────────────────────────────────────────────────
    $('#studentFilter, #actionFilter, #moduleFilter, #dateFrom, #dateTo').on('change', function () {
        dataTable.draw();
    });

    $('#clearFilters').on('click', function () {
        $('#studentFilter, #actionFilter, #moduleFilter').val('');
        $('#dateFrom, #dateTo').val('');
        dataTable.draw();
    });

    // ── Detail Modal ─────────────────────────────────────────────────────────
    $('#studentAuditTable').on('click', '.view-detail', function () {
        ...
        $.ajax({
            ...
            success: function (log) {
                ...
                $('#detail-student').text(log.user_identifier || 'N/A');
