...
class TestDevController extends Controller
{
    ...
    {
        return view('testdev.test-date');
    }

    public function toggle_dev_flag(Request $request)
    {
        $request->validate([
            'flag'  => 'required|string|in:bypass_date_check',
            'value' => 'required|boolean',
        ]);

        Cache::put($request->flag, (bool) $request->value, now()->addHours(24));
    ...
    public function get_guardians()
    {
        $guardians = DB::table('guardians as g')
            ...
            ->groupBy(
                'g.id', 'g.email', 'g.first_name', 'g.last_name', 'g.access_token',
    ...
    public function set_mock_time(Request $request)
{
    $request->validate([
    ...
    if ($request->datetime) {
        Cache::put('dev_mock_time', $request->datetime, now()->addHours(24));
    } else {
        Cache::forget('dev_mock_time');
    }

    return response()->json(['success' => true, 'mock_time' => $request->datetime]);
}
}